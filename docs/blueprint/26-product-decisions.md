...
## Initial Business Observation and Analytics Decision — 2026-08-08

Business Observation is a read-only, eventually consistent derived capability. It reports aggregate Membership, Receipt, Reward Point, Redemption, Engagement, and Status metrics from authoritative historical records and never becomes the source of truth for transactional domains. Initial metrics use explicit UTC half-open periods, preserve currency separation, retain configuration-version attribution, suppress Customer PII, and exclude revenue attribution, ROI, lifetime value, churn, cohorts, campaign attribution, liability, forecasting, and AI recommendations. Domain/query contracts and deterministic in-memory projections may be implemented now; production persistence, materialized views, and event consumers remain deferred to infrastructure work.
