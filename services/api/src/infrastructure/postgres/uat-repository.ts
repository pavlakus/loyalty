...
import type { LocalMvpScenarioResult } from "../../application/local-mvp-ports.js";
import type { PostgresTransactionManager } from "./transaction-context.js";
import type { UatFixture, UatIdentity, UatReceiptPersistenceInput, UatRepositoryPort } from "../../application/uat-repository-port.js";

function digest(token: string): string { return createHash("sha256").update(token).digest("base64url"); }

export class PostgresUatRepository implements UatRepositoryPort {
  public constructor(
  ...
  public async submitReceipt(input: UatReceiptPersistenceInput): Promise<LocalMvpScenarioResult> {
    if (input.identity.kind !== "customer" || !input.identity.customerId) throw new Error("Customer authentication is required");
    const amountMinor = input.amountMinor;
    const { pointsEarned, xpEarned } = input;
